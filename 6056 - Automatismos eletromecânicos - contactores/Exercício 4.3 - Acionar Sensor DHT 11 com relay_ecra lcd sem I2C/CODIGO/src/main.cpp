...
void loop() {
  ...
  if (temperatura > T0 && temperatura < T1) {
    if (!aquecimento) {
      aquecimento = true;
      digitalWrite(RELAY_PIN, LOW);
      Serial.println(F("AQUECIMENTO DESLIGADO - Temperatura fora da faixa"));
    }
  } else {
    if (aquecimento) {
      aquecimento = false;
      digitalWrite(RELAY_PIN, HIGH);
      Serial.println(F("AQUECIMENTO LIGADO - Temperatura entre T0 e T1"));
    }
  }
